      ...
      <div class="head_top">
          ...
          <div class="topmenu">
            <ul>
				...
				<div class="gnb">
					<div class="conwrap">
						<h1>
							<a href="${cp}/sanghoon/index.jsp"><img
							...
							<ul>
								<li><a href="${cp}/kyungchul/ceo.jsp">모나미소개</a>
									<ul>
										...
										<li><a href="http://recruit.monami.co.kr/"
								...
								<li><a href="${cp}/jungmin/pen/penpage.jsp">모나미제품</a>
									<ul>
										...
										<li><a href="${cp}/jungmin/marker/markerpage.jsp">마카</a>
										</li>
								...
								<li><a href="${cp}/teamin/monami_news1.jsp">NEWS &
										VIDEO</a>
									<ul>
		...
		<div id="container">
			<div class="contents">
				...
				<div class="pagetabs">
					<ul class="m5">
						<li class="on"><a href="${cp}/teamin/monami_news1.jsp">보도자료</a>
						</li>
						<li><a href="${cp}/teamin/monami_video1.jsp">동영상자료</a></li>
					</ul>
				</div>
				...
				<form>
					<table class="bbs_view" cellpadding="0" cellspacing="0">
						<tbody>
							...
							<tr>
								<td class="viewcon"><c:set var="news_imgctt"
										value="${board.news_imgctt.split(\";\")}" /> <c:set
										var="news_content" value="${board.news_content.split(\";\")}" />

									<p style="text-align: center;" align="center">
										<span style="font-size: 18pt;"> <b>${board.news_subtit}</b>
										</span>
									</p>
									...
									<p style="text-align: center;" align="center">
										...
										<c:choose>
											<c:when test="${files != null and files.size()>0 }">
												<img
													src="${cp}/news/FileDownload.mo?systemname=${files[0].systemname}&orgname=${files[0].orgname} title="page_1.jpg">
											</c:when>
											<c:otherwise>
												<img src="${cp}/images/no-img.gif">
											</c:otherwise>
										</c:choose>
										<br style="clear: both;"> &nbsp;

									</p> <c:forEach var="i" begin="0" end="${fn:length(news_imgctt)}"
										step="1">
										<p style="text-align: center;" align="center">
											<span style="font-size: 10pt;">${news_imgctt[i]}&nbsp;
											</span>
										</p>
									</c:forEach>
									<p>&nbsp;</p>
									<p>&nbsp;</p> <c:choose>
										<c:when
											test="${fn:length(news_content)==2 || fn:length(news_content)==1}">
											<p>
												<c:forEach var="z" begin="0"
													end="${fn:length(news_content)}" step="1">
													<span style="font-size: 10pt;">${news_content[z]}&nbsp;</span>
												</c:forEach>
											</p>
										</c:when>
										<c:otherwise>
											<c:forEach var="j" begin="0"
												end="${fn:length(news_content)-2}" step="1">
												<p>
													<span style="font-size: 10pt;">${news_content[j]}&nbsp;</span>
												</p>
												<p>&nbsp;</p>
												<p>
													<span style="font-size: 10pt;">${news_content[fn:length(news_content)-1]}&nbsp;</span>
												</p>
											</c:forEach>
										</c:otherwise>
									</c:choose>
				...
				<form name="removeForm" action="${cp}/news/NewsRemove.mo" method="get">
						<input type="hidden" name="news_idx" value="${board.news_idx}">
					</form>
				<div class="view_btns">
					<a href="${cp}/news/NewsModify.mo?news_idx=${board.news_idx}">수정</a>&nbsp;&nbsp;
					<a href="javascript:document.removeForm.submit()">삭제</a>&nbsp;&nbsp;
					<!--get방식으로 form 제출시 안의 input-->
					<a
						href="${cp}/news/NewsBoardList.mo?page=${param.page == null ? 1 : param.page}">LIST</a>
					<div class="view_page">
						<c:choose>
							<c:when test="${nidx != null && pidx == null }">
								<a href="${cp}/news/News_page.mo?news_idx=${nidx}"> <img
									src="${cp}/monami_images/view_page_next.gif" alt="">
								</a>
							</c:when>
							<c:when test="${nidx != null && pidx != null }">
								<a href="${cp}/news/News_page.mo?news_idx=${pidx}"> <img
        ...
        개인정보취급방침 시행일자 : 2018년 08월 02일 
					...
					<div class="email" style="display: none;">
						...
						<div class="textarea">
							...
							거부하며, 이를 위반시정 보통신망법에 의해 형사 처벌됨을 유념하시기 바랍니다. <br> <br>
				...
				<div class="foot_btm">
					...
					<div class="fam_site">
						...
						<ul>
							...
							<li><a href="http://www.monamicosmetics.com/"
								target="_blank">모나미코스메틱</a></li>
						</ul>
					</div>
				</div>
			</div>
